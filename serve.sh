...
for ((i = 0; i < $NUM_WORKERS; i++)); do
  env DEVICE=${DEVICE-cuda:$i} GROUP=${GROUP-group$i} /bin/bash -c 'python examples/pytorch/gptj/app/serving.py --ckpt_path /together/together_models/GPT-JT-6B-v1-tp1' &
done
